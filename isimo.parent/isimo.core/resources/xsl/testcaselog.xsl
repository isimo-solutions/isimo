...
<xsl:template match="/">
  <HTML>
    <HEAD>
      ...
      <!-- element container -->
        .e_failure  {margin-left:1em; text-indent:-1em; margin-right:1em; background-color:yellow}
      <!-- element container -->
        .e_error  {margin-left:1em; text-indent:-1em; margin-right:1em; background-color:orange}
        
        .description  {color:darkgreen; background-color:lightyellow;  margin-left:0.5em}  
      <!-- comment or cdata -->
...
<xsl:template match="@*" xml:space="preserve"><SPAN>
<xsl:choose>
<xsl:when test="false()"></xsl:when>
<xsl:otherwise><xsl:attribute
name="class"><xsl:if
test="starts-with(name(),'xsl:')">x</xsl:if>t</xsl:attribute></xsl:otherwise></xsl:choose>
<xsl:value-of
select="concat(' ',name())"/></SPAN><SPAN class="m">="</SPAN>
	  <B><xsl:value-of
...
<xsl:call-template name="description"/>
  <DIV><xsl:call-template name="errorclass"/><DIV STYLE="margin-left:1em;text-indent:-2em">
  <SPAN class="b">&#160;</SPAN>
...
<xsl:template match="*[comment() | processing-instruction()]">
  <DIV><xsl:call-template name="errorclass"/>
...
<xsl:call-template name="description"/>
  <DIV>
  	<xsl:call-template name="errorclass"/>
...
<xsl:template name="errorclass">
  	<xsl:choose>
  	<xsl:when test="*[name()='error']">
      <xsl:attribute name="class">e_error <xsl:value-of select="name()"/></xsl:attribute>
  	</xsl:when>
    <xsl:when test="*[name()='failure']">
      <xsl:attribute name="class">e_failure <xsl:value-of select="name()"/></xsl:attribute>
  	</xsl:when>
  	<xsl:otherwise><xsl:attribute name="class">e</xsl:attribute></xsl:otherwise>
  	</xsl:choose>
</xsl:template>

<xsl:template name="description">
<xsl:if test="@description">
<DIV class="description">&#160;&#160;&#160;<xsl:value-of select="@description"/>
</DIV>
</xsl:if>
</xsl:template>
